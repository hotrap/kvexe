...
public:
	RouterVisCnts(
		...
		buffered_channel<std::pair<std::string, std::string>> *promote_chan
	) : switches_(switches),
	...
	void Access(int level, rocksdb::Slice key, rocksdb::Slice value) override {
...
void promoter_fn(
	rocksdb::DB *db, buffered_channel<std::pair<std::string, std::string>> *kvs,
	size_t *promote_count
) {
	for (auto kv : *kvs) {
		++*promote_count;
		rusty_assert(
			db->Put(rocksdb::WriteOptions(), kv.first, kv.second).ok()
		);
	}
}
