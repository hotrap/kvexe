...
class Tester {
 public:
  ...
  void print_other_stats(std::ostream &log) {
    ...
    auto cache_stats = options_.cache->getGlobalCacheStats();
  ...
  void print_heavy_stats(std::ostream &out) {
    auto stats = options_.cache->getAccessContainerDistributionStats();
 ...
 private:
  ...
  void finish_load_phase(const rusty::sync::Mutex<std::ofstream> &info_json_out,
    ...
    *info_json_out.lock() << "\t\"load-wait-time(secs)\": "
    ...
    print_heavy_stats(other_stats_out);
  }
...
int main(int argc, char **argv) {
  ...
  desc.add_options()(
      "run_70p_ops",
      po::value<uint64_t>(&work_options.run_70p_ops)->default_value(0),
      "OPS in the first 70% of the run phase. 0 means unlimited.");
